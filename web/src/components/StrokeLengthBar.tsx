/**
 * Barre de longueur de coup — pattern Peach/FM (brief §1).
 *
 * Lecture : blanc début = catch mou ; gris = palette qui travaille ;
 * blanc fin = washing out. Longueur = course relative (arc / nominal).
 */
...
export type StrokeLengthBarProps = {
  ...
  seat?: number;
  label?: string;
  /** Largeur max de la piste (px CSS). */
...
export function StrokeLengthBar({
  ...
  return (
    <div
      ...
      style={{ maxWidth }}
      title={
        `catch blanc ${(100 * c).toFixed(0)}% · immergé ${(100 * m).toFixed(0)}% · ` +
        `sortie ${(100 * f).toFixed(0)}% · longueur ×${len.toFixed(2)}`
      }
    >
      {(seat != null || label) && (
        <div className="stroke-bar-label">
          {label ?? `Poste ${seat}`}
        </div>
      )}
      <div className="stroke-bar-track-wrap">
        ...
        >
          <span
            className="stroke-bar-seg white"
            style={{ flex: c / sum }}
            aria-label="catch"
          />
          <span
            className="stroke-bar-seg immersed"
            style={{ flex: m / sum }}
            aria-label="immergé"
          />
          <span
...
/** Empilement vertical — une barre par poste (Vue Équipage / Coach). */
  ...
  return (
    <div className="stroke-bar-stack">
      <p className="muted stroke-bar-legend">
        Blanc début = catch mou · Gris = palette qui travaille · Blanc fin =
        washing out
      </p>
      {bars.map(({ seat, bar }) =>
        bar ? (
