"""Classes de bateau et loi d'échelle §2.4 (calibrée sur le 8+ uniquement).

Les valeurs `k_drag` / `S_wet` / `CdA` des YAML restent la source frozen pour
la simulation. `scale_from_8plus` sert aux tests de cohérence (§9.3) — elle
ne lit jamais les 7 autres classes.
"""
...
def scale_from_8plus(M_tot: float) -> dict[str, float]:
    """Dérive k_drag, S_wet, CdA de M_tot seul, calibré uniquement sur le 8+.
...
def total_mass_kg(params: dict[str, Any]) -> float:
    ...
    boat = params["boat"]
    crew_masses = params["crew"]["mass_kg"]
    return (
        float(boat["m_hull_kg"])
        + float(sum(crew_masses))
