"""Métriques visuelles coaching (brief-visuels-coaching-enrichis.md).

Pas de nouvelle physique — dérivé de Stroke + params. Seuil de force
pour la portion immergée = 10 % du pic (convention lecture barre Peach/FM).
"""
...
FORCE_WORK_FRAC = 0.10  # F > 10 % Fmax → portion « immergée / travaillante »
...
def stroke_length_bar(
    ...
    *,
    ...
    """Barre de longueur de coup (brief §1).

    - ``length_norm`` : arc réalisé / arc nominal (proxy course de coulisse)
    - ``catch_white`` : fraction avant travail effectif (slip / catch mou)
    - ``immersed`` : fraction où F > 10 % Fmax (palette qui travaille)
    - ``finish_white`` : fraction après fin de travail (washing out)
    """
    ...
    thr = FORCE_WORK_FRAC * Fmax
    working = drive & (np.abs(F) > thr)
    if working.any():
        u0 = float(np.min(u[working]))
        u1 = float(np.max(u[working]))
    else:
        u0, u1 = 0.0, 0.0
    catch_white = float(np.clip(u0, 0.0, 1.0))
    finish_white = float(np.clip(1.0 - u1, 0.0, 1.0))
    immersed = float(np.clip(u1 - u0, 0.0, 1.0))
    # renormalise (sécurité numérique)
    ...
    if drive.any():
        realized = float(np.max(th[drive]) - np.min(th[drive]))
    else:
        realized = 0.0
...
def crew_coaching_payload(st: Any, P: dict[str, Any]) -> list[dict[str, Any]]:
    """Enrichit chaque poste : barre + séries drive + poisson."""
    ...
    for i in range(n):
        ...
        bar = stroke_length_bar(
            st.theta[i], st.handle_force[i], st.immersion[i],
            th_catch=th_c, th_finish=th_f, L_slide_m=L_slide,
        )
        drive = drive_series(
            st.theta[i], st.handle_force[i], st.immersion[i], st.theta_dot[i],
            th_catch=th_c, th_finish=th_f,
        )
