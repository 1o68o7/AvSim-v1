...
Ne pas assouplir un seuil. Sur 8+/1x, η et check_factor hors bande sont
des écarts connus du modèle 1DOF (STATE.md / ROADMAP « Piste différée »)
→ xfail documenté. Sur les 6 classes bêta, un échec est une découverte
à rapporter (Phase 1 : échouer pour de vraies raisons).
"""
...
def test_check_factor_intra_stroke(metrics) -> None:
    """cf peak-to-peak : bande §9.2 ; hors bande 8+/1x = limite 1DOF connue."""
    code = metrics["code"]
    lo, hi = check_factor_band(code)
    cf = metrics["check_factor"]
    ok = lo <= cf <= hi
    if code in KNOWN_CF_GAP_CLASSES and not ok:
        pytest.xfail(
            f"{code}: check_factor={cf:.3f} hors [{lo},{hi}] — limite 1DOF "
            f"connue (empilement CdM × Hermite² ; levier largeur épuisé)."
        )
    assert ok, (
